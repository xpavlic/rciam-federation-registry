...
FROM
    (SELECT id FROM client_details_saml WHERE entity_id=${entity_id} and id!=${client_id}) as t1
...
FROM
	(SELECT id FROM client_petition_details_saml WHERE entity_id=${entity_id} and id!=${client_petition_id}) as t1
