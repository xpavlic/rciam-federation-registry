...
FROM (SELECT *, COUNT(*) OVER() AS full_count,COUNT(CASE WHEN outdated = true AND petition_id IS NULL AND bar.owned THEN 1 ELSE null END) OVER() AS outdated_count,COUNT(CASE WHEN status = 'request_review' THEN 1 ELSE null END) OVER()
    ...
    FROM (
        SELECT DISTINCT
            ...
            deployment_type,
            CASE WHEN petitions.last_edited IS NOT NULL THEN petitions.last_edited ELSE service_state.last_edited END,
            outdated,
            client_id,
            entity_id,
            created_at,
        ...
        (
            ...
            WHERE tenant = ${tenant_name} ${protocol_filter:raw} ${integration_environment_filter:raw}
        ) AS service_details
        LEFT JOIN service_details_saml ON service_details.service_id = service_details_saml.id
        LEFT JOIN service_details_oidc ON service_details.service_id = service_details_oidc.id
        ${select_all:raw}
        ...
        FROM (
            SELECT
                ...
                ()
            FROM service_petition_details
            WHERE reviewed_at IS NULL AND type = 'create' AND tenant = ${tenant_name} ${protocol_filter:raw} ${integration_environment_filter:raw} ${outdated_disable_petitions:raw} ${pending_sub_filter:raw} ${disable_petitions:raw}
        ) as petitions
        LEFT JOIN service_petition_details_oidc ON petitions.petition_id = service_petition_details_oidc.id
        LEFT JOIN service_petition_details_saml ON petitions.petition_id = service_petition_details_saml.id
        LEFT JOIN group_subs ON petitions.group_id = group_subs.group_id AND group_subs.sub = ${sub}
